...
VisSense.fn.onPercentageTimeTestPassed = function (callback, config) {
  ...
  var outerMonitor = VisSense.of(this._element, {
    // monitor is considered hidden if it is 1% below the percentage limit
    hidden: _config.percentageLimit - 0.01
  }).monitor({
    strategy: _config.strategy,
    update: function (monitor) {
      console.log('[outerMonitor.update] ' + monitor.state().state);
    },
    fullyvisible: function () {
      console.log('[outerMonitor.fullyvisible]');
    },
    visible: function (monitor) {
      ...
      if (inner.monitor === null) {
        ...
        inner.monitor = createInnerMonitor(monitor, callback, _config);
      }
