...
var onPercentageTimeTestPassed = function (element, callback, config) {
  ...
  var outerMonitor = VisSense.VisMon.Builder(new VisSense(element, {
    hidden: hiddenLimit
  }))
...
VisSense.fn.on50_1TestPassed = function (callback, options) {
  ...
  /*
   var outerMonitor = VisSense.VisMon.Builder(this)
   .strategy(new VisSense.VisMon.Strategy.PercentageTimeTestEventStrategy('ptt-50/1-passed', config))
   .on('ptt-50/1-passed', function(monitor, data) {
   callback(monitor, data);
   outerMonitor.stop();
   })
   .build();
